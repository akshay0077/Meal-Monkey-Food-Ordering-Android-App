    ...
    if(isset($_POST['submit']))
    {
       if(empty($_POST['catname']))
       {
        $categoryerror['error']['catname'] = "please enter category name";
           ...
           $checkcategory = "select * from cat where name = '$catname'";
           ...
           if($catcount > 0 )
           {
               ...
               if(isset($_FILES['image']['name']))
               {
                ...
                {
                    $categoryerror['error']['image']= "please select Image";
                }
                else{
                    if ($file_ex == '.JPG' || $file_ex == 'JPEG' || $file_ex == '.PNG') {
                        if (empty($categoryerror['error']['image'])) {
                            ...
                            move_uploaded_file($file_path, 'images/category/'.$img_name);
                            $insertcat = "insert into cat (name , img) values ('$catname' , '$img_name')";
